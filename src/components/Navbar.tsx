...
// export default Navbar;import React, { useState, useEffect, useRef } from "react";import { Link, useLocation } from "react-router-dom";
import { Menu, X, ChevronDown } from "lucide-react";
import { menuItems } from "../data/menuItems";
import coyoliaLogo from "../assets/CoyoliaPrimarylogo.avif";
...
const Navbar: React.FC = () => {
  ...
  // Ref to store timeout ID for delayed submenu close
  ...
  // On mouse enter, clear any close timeout and open submenu immediately
  ...
  // On mouse leave, start timeout to close submenu after 3 seconds
              ...
              >
                {item.submenu ? (
                  <>
                    ...
                    <div
                      className={`absolute left-0 mt-2 w-56 bg-white text-[#21204C] rounded-md shadow-lg z-10
                        transition-opacity duration-300 ease-in-out
                        ${activeSubmenu === item.title ? "opacity-100 visible pointer-events-auto" : "opacity-0 invisible pointer-events-none"}`}
                    >
                      {item.submenu.map((subItem) => (
                        <Link
                          key={subItem.path}
